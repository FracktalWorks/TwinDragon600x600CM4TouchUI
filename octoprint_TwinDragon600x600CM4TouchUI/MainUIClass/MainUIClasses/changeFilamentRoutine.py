...
class changeFilamentRoutine(mainGUI.Ui_MainWindow):
    ...
    def unloadFilament(self):
        ...
        if self.changeFilamentComboBox.findText("Loaded Filament") == -1:
            self.octopiclient.setToolTemperature({"tool1": filaments[str(
                self.changeFilamentComboBox.currentText())]}) if self.activeExtruder == 1 else self.octopiclient.setToolTemperature(
